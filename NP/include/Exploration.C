void BDT::Exploration_k(int k, TString TData, TString TSim,TCut cutSB, TString extension){
  ...
  TH1F *hist2c_Sim = new TH1F("hist2c_Sim","Histogram",100,0,5);
  ...
  TH1F *hist2c_Data = new TH1F("hist2c_Data","Histogram",100,0,5);
  ...
  c1->Print(in+Form("_k=%i_BDT=%f_entries=%f",k,BDTCut[k],hist1c_Data->GetEntries())+extension);
