...
float phi, xB_meas, Q2_meas, t_meas, phi_meas;
    ...
    while (std::getline(infile, line)) {
        ...
        for (int i=0; i < numParticles; i++) {
            ...
            if (pid == 11) {  // Electron
                ...
                xB = x2;  // xBj is in the second column of the electron
                Q2 = x10;       // Q^2 is in the tenth column of the electron
                t = -x11;       // -t is in the eleventh column of the electron
            } 
	    ...
	    phi_meas= 180. / TMath::Pi() * leptoP.Angle(hadroP);
	    
	    if (leptoP.Dot(ph_vec) < 0.)
	      phi_meas = 360. - phi_meas;
...
int lund2root_RC() {
    //int N0=0; //DVCS RC
    int N0=10000; //DVCS RC flat
    //int N0=12000; //BH RC flat
    //int N0=14000; //DVCS flat
    //int N0=16000; //BH flat
    
    //const char* outputFileName = "dvcsgen_RC_gen.root";
    const char* outputFileName = "dvcsgen_test.root";
    //const char* outputFileName = "dvcsgen_BH_gen.root";
    //const char* outputFileName = "dvcsgen_DVCS_noRC_gen.root";
    //const char* outputFileName = "dvcsgen_BH_noRC_gen.root";
    
    const int N=2000; //out of 10000
    ...
    tree->Branch("phi", &phi);
    tree->Branch("xB_meas", &xB_meas);
    tree->Branch("Q2_meas", &Q2_meas);
    tree->Branch("t_meas", &t_meas);
    tree->Branch("phi_meas", &phi_meas);
    tree->Branch("theta_gamma_e", &theta_gamma_e);

    for(int i=1; i<=1; i++)
    {
        //printProgress((i-N0)*1.0/N);
        createTTreeFromLund("dvcsgen1.dat", tree);
    }
