TH1F* BDT::BM_correction(TH1F*& hist, int bin, int select){
  gStyle->SetOptStat(0);
  gStyle->SetOptFit(0);
  ...
  while (std::getline(inputFile, line)) {
    ...
    hist->SetBinError(hbin, sqrt(pow(xErr,2) + pow(yErr,2))); //Set the error of the histogram to the sum of the errors from the BM correction
  ...
  for (int k=1; k<=Nphi; k++)
    std::cout<<" "<<ratio->GetBinContent(k)<<" "<<ratio->GetBinError(k)<<" "<<hist->GetBinContent(k)<<" "<<BM_corr->GetBinContent(k)<<std::endl;
