TH1* BDT::Get_Contamination_Mostafa(TCut cut, double BDT_cut, int Nphi, bool eta=false)
{
  ...
  if(temps == "FT/")
    String_cut.ReplaceAll("strip_Ph_Theta", "mm2_eNgg");//mm2_eNgg for FT and theta_eta_e for FD
  if(temps == "FD/")
    String_cut.ReplaceAll("strip_Ph_Theta", "theta_eta_e");//mm2_eNgg for FT and theta_eta_e for FD
  ...
  cut2g = TCut(String_cut)+ TCut("strip_eta_2DChi2 < 0.04 && abs(mm2_eNgg)<0.01") + TCut("mm2_egg<1.5") ; // && (strip_Ph1_Theta <5 || strip_Ph2_Theta <5)");
  }
  else
  {
  ...
  String_cut.ReplaceAll("Phi_Ph", "Phi_Pi0");  
  String_cut.ReplaceAll("N_Ph", "mm2_eNgg");
  ...
  //Create cut for 2gamma case
  cut2g = TCut(String_cut)+ TCut("strip_Pi0_2DChi2 < 0.04 && abs(mm2_eNgg)<0.01") + TCut("mm2_egg<1.5") ; // && (strip_Ph1_Theta <5 || strip_Ph2_Theta <5)");
  }
  ...
  if(generate_most)
    {
      ...
      Pi02g->Project("Pi02g_p", branch, cut2g );
      Pi02g->Project("Pi02g_m", branch, cut2g );
  
      Pi02gData->Project("Pi02g_Data_p", branch, cut2g + TCut("Helicity>0") );
      Pi02gData->Project("Pi02g_Data_m", branch, cut2g + TCut("Helicity<0") );
      ...
      if (Pi01g->GetEntries() < 1) {
      TFile* out_sim_p=new TFile(Folder + TString("TMostafa_pi0.root"), "RECREATE");
      ...
      for(int t=0; t<tree_m->GetEntries();t++)
	{
      ...
      Add_BDT_var_Formula(cut, Folder + output, TString("T") + output, Vars);
