...
void BDT::Single_BSA_3(TString BDT_Data, TString Bin_Data, vector<double> boundaries, int P_bins=12)
{
  ...
  ch3->Project("tmean","t_Ph",cutM + TCut(Form("t_Ph > %f && t_Ph < %f && strip_Q2 > %f && strip_Q2 < %f && strip_Xbj > %f && strip_Xbj < %f",boundaries.at(0),boundaries.at(1),boundaries.at(2),boundaries.at(3),boundaries.at(4),boundaries.at(5))));
  meant=tmean->GetMean();

  TH1F *Qmean = new TH1F("Qmean","Q_temp",100,0,0);
  ch3->Project("Qmean","strip_Q2",cutM + TCut(Form("t_Ph > %f && t_Ph < %f && strip_Q2 > %f && strip_Q2 < %f && strip_Xbj > %f && strip_Xbj < %f",boundaries.at(0),boundaries.at(1),boundaries.at(2),boundaries.at(3),boundaries.at(4),boundaries.at(5))));
  meanQ=Qmean->GetMean();

  TH1F *xmean = new TH1F("xmean","x_temp",100,0,0);
  ch3->Project("xmean","strip_Xbj",cutM + TCut(Form("t_Ph > %f && t_Ph < %f && strip_Q2 > %f && strip_Q2 < %f && strip_Xbj > %f && strip_Xbj < %f",boundaries.at(0),boundaries.at(1),boundaries.at(2),boundaries.at(3),boundaries.at(4),boundaries.at(5))));
  meanx=xmean->GetMean();
