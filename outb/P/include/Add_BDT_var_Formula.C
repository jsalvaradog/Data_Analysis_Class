...
void BDT::Add_BDT_var_Formula(TCut cutSB, TString Data, TString output, vector<TString> vars)
{
  ...
  reader1->AddVariable("mm2_eNg", &_mm2_eNg1);
  reader1->AddVariable("mm2_eg", &_mm2_eg1);
  reader1->AddVariable("delta_Phi", &_delta_Phi1);
  ...
  Tree->SetBranchAddress("mm2_eNg", &_mm2_eNg);
	...
	if(flag->at(i)==1)
	  {
	    ...
	    _mm2_eNg1 = _mm2_eNg->at(i);
	    //std::cout<<_mm2_eNg1<<endl;
	    
	    _mm2_eg1 = _mm2_eg->at(i) ;
	    //std::cout<<_mm2_eg1<<endl;
	    
	    _delta_Phi1 = _delta_Phi->at(i);
