...
void BDT::Compute_BM_Matrix() 
{
      ...
      for(int j=0; j<flag->size(); j++)
	{
	  if(flag->at(j)==1 && BDTvar > BDT_value)
	      ...
	      for(int k=0; k<64; k++)
		{
	      ...
	      if(abs(binx-biny)<99)//Only 2 bins away
			{
  ...
  cout << "\n==================================== UNFOLD ===================================" << endl;
  RooUnfoldSvd   unfold_p (&response, input_p);
  TH1D* output_p= (TH1D*) unfold_p.Hunfold();

  RooUnfoldSvd   unfold_m (&response, input_m);
  TH1D* output_m= (TH1D*) unfold_m.Hunfold();

  RooUnfoldSvd   unfold_p_2 (&response, input_p_2);
  TH1D* output_p_2= (TH1D*) unfold_p_2.Hunfold();

  RooUnfoldSvd   unfold_m_2 (&response, input_m_2);
