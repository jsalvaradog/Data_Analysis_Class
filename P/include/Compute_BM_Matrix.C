...
void BDT::Compute_BM_Matrix() 
{
  ...
  int binx, biny;
  int col, row;
  TTreeFormula* formula = new TTreeFormula("cutFormula", (cut + TCut(Form("_strip_Nuc_BDT>%f",BDT_value))).GetTitle(), Data);
  ...
  TString TBM_Sim_noRec = TString("/volatile/clas12/jsalvg/simulation/dvcsgen/inb/withRC/MCnoRec/Quality_DVCS_noRec.root");
