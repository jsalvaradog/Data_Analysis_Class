...
TH1* BDT::Maxime(TCut cut, double BDT_cut, int bin_number, int Nphi)
{
  ...
  for (int i = 1; i <= int(sizeof(Mbins) / sizeof(Mbins[0])) ; i++) {
    ...
    if (!fileStream.is_open() && !fileStream_r.is_open() && false) 
      {
  ...
  if(cut_sys_ || pid_sys)
  {
        gSystem->Exec(TString("cp ") + dir + TString(Form("Analysis/bin_%i/TMaxime_pi0.root ",bin_number)) + TString(" ") + Folder + TString("TMaxime_pi0_0.root"));
        Add_BDT_var_float(cut, Folder + TString("TMaxime_pi0_0.root"), TString("TMaxime_pi0.root"));
  }
