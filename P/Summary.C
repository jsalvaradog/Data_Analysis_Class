...
//=========  (Thu Jan  5 15:05:52 2023) by ROOT version 6.26/10
   ...
   TFile *File = new TFile("Analysis/TMVACC.root");   
...
void Kinematics(TTree* pDVCS, TTree* pDVCS_bkg){
  ...
  pDVCS->Project("histeT", "strip_El_P:strip_El_Theta",cut_NP + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)"));
  pDVCS->Project("histgT", "strip_Ph_P:strip_Ph_Theta",cut_NP + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)"));
  pDVCS->Project("histNT", "strip_Nuc_P:strip_Nuc_Theta",cut_NP + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)"));
  pDVCS->Project("histeP", "strip_El_P:strip_El_Phi",cut_NP + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)"));
  pDVCS->Project("histgP", "strip_Ph_P:strip_Ph_Phi",cut_NP + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)"));
  pDVCS->Project("histNP", "strip_Nuc_P:strip_Nuc_Phi",cut_NP + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)"));

  pDVCS_bkg->Project("histeT_bkg", "strip_El_P:strip_El_Theta",(cut_NP + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)"))*TCut("Weight"));
  pDVCS_bkg->Project("histgT_bkg", "strip_Ph_P:strip_Ph_Theta",(cut_NP + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)"))*TCut("Weight"));
  pDVCS_bkg->Project("histNT_bkg", "strip_Nuc_P:strip_Nuc_Theta",(cut_NP + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)"))*TCut("Weight"));
  pDVCS_bkg->Project("histeP_bkg", "strip_El_P:strip_El_Phi",(cut_NP + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)"))*TCut("Weight"));
  pDVCS_bkg->Project("histgP_bkg", "strip_Ph_P:strip_Ph_Phi",(cut_NP + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)"))*TCut("Weight"));
  pDVCS_bkg->Project("histNP_bkg", "strip_Nuc_P:strip_Nuc_Phi",(cut_NP + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)"))*TCut("Weight"));
...
void Phase_Space(TTree* pDVCS, TTree* pDVCS_bkg)
{
  ...
  TCut TheCut = cut_NP + TCut("abs(t_Ph/strip_Q2)<1 && N_Ph<3 && t_Ph>-1") + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)");
...
TGraph* Recreate_ROC_Sim(TTree* signal_DVCS, TTree* signal_Pi){
  vector<double> Sig_Acc;
  vector<double> Bac_Rej;
  Sig_Acc.clear();
  Bac_Rej.clear();
  ...
  float cut;
      ...
      TCut TheCut0 =TCut("bestCandidateFlag==1 && strip_Xbj <1 && strip_Xbj >0 && t_Ph <0 && strip_Q2 > 1.0 && strip_W > 2 && strip_Nuc_P > 0.35 && strip_El_P > 1.0 && strip_Ph_P>2  && strip_El_vz < 10 && strip_El_vz > -12 && TMath::Abs(Phi_Nuc - Phi_Ph) < 2 && TMath::Abs(t_Nuc - t_Ph) < 2 && TMath::Sqrt(Xbal * Xbal + Ybal*Ybal + Zbal*Zbal) <1");
      
      TH1F *Cutted_DVCS0 = new TH1F("Cutted_DVCS0", "Data10", 100,0,0);
      signal_DVCS->Project("Cutted_DVCS0", "_strip_Nuc_BDT",TheCut0);
 
      TH1F *Cutted_Pi0 = new TH1F("Cutted_Pi0","Data20",100,0,0);
      signal_Pi->Project("Cutted_Pi0", "_strip_Nuc_BDT",TheCut0);

  int DVCS_tot=Cutted_DVCS0->GetEntries();
  int Pi_tot=Cutted_Pi0->GetEntries();

 
  for(int j=0; j<=points;j++)
    {
      cut=-0.4 + 0.8*j/points;
      TCut TheCut =TCut(Form("_strip_Nuc_BDT > %f && bestCandidateFlag==1 && strip_Xbj <1 && strip_Xbj >0 && t_Ph <0 && strip_Q2 > 1.0 && strip_W > 2 && strip_Nuc_P > 0.35 && strip_El_P > 1.0 && strip_Ph_P>2  && strip_El_vz < 10 && strip_El_vz > -12 && TMath::Abs(Phi_Nuc - Phi_Ph) < 2 && TMath::Abs(t_Nuc - t_Ph) < 2 && TMath::Sqrt(Xbal * Xbal + Ybal*Ybal + Zbal*Zbal) <1",cut));


      TH1F *Cutted_DVCS = new TH1F("Cutted_DVCS", "Data1", 100,0,0);
      signal_DVCS->Project("Cutted_DVCS", "_strip_Nuc_BDT",TheCut);
 
      TH1F *Cutted_Pi = new TH1F("Cutted_Pi","Data2",100,0,0);
      signal_Pi->Project("Cutted_Pi", "_strip_Nuc_BDT",TheCut);
     
      Bac_Rej.push_back((Pi_tot-Cutted_Pi->GetEntries())/Pi_tot);
      Sig_Acc.push_back(Cutted_DVCS->GetEntries()/DVCS_tot);

      std::cout<<j<<" "<<Cutted_DVCS->GetEntries()/DVCS_tot<<" "<<(Pi_tot-Cutted_Pi->GetEntries())/Pi_tot<<endl;
  ...
  TGraph *gr = new TGraph(points+1,Sig_Acc.data(),Bac_Rej.data());
...
TGraph* Recreate_ROC_Data(TTree* signal_Data){
  vector<double> Sig_Acc;
  vector<double> Bac_Rej;
  Sig_Acc.clear();
  Bac_Rej.clear();
  ...
  float cut;
      ...
      TH1F *Cutted_DVCS0 = new TH1F("Cutted_DVCS0", "Data10", 100,0,0);
      signal_Data->Project("Cutted_DVCS0", "_strip_Nuc_BDT",TheCut0 + TCut("_strip_Nuc_BDT > 0.0"));
 
      TH1F *Cutted_Pi0 = new TH1F("Cutted_Pi0","Data20",100,0,0);
      signal_Data->Project("Cutted_Pi0", "_strip_Nuc_BDT",TheCut0  + TCut("_strip_Nuc_BDT < 0.0"));
  ...
  for(int j=0; j<=points;j++)
    {
      ...
      TH1F *Cutted_DVCS = new TH1F("Cutted_DVCS", "Data1", 100,0,0);
      signal_Data->Project("Cutted_DVCS", "_strip_Nuc_BDT",TheCut  + TCut("_strip_Nuc_BDT > 0.0") );
 
      TH1F *Cutted_Pi = new TH1F("Cutted_Pi","Data2",100,0,0);
      signal_Data->Project("Cutted_Pi", "_strip_Nuc_BDT",TheCut + TCut("_strip_Nuc_BDT < 0.0"));
...
TGraph* Recreate_ROC_Training(){
  ...
  TChain *signal_Data= new TChain("dataset/TrainTree");
  for(int i=1; i<=96; i++)
  {
  	signal_Data->Add(Form("bin_%i/TMVACC.root",i));
  }
  ...
  float cut;
      ...
      TH1F *Cutted_DVCS0 = new TH1F("Cutted_DVCS0", "Data10", 100,0,0);
      signal_Data->Project("Cutted_DVCS0", bdtvar, TCut("classID==0"));
 
      TH1F *Cutted_Pi0 = new TH1F("Cutted_Pi0","Data20",100,0,0);
      signal_Data->Project("Cutted_Pi0", bdtvar, TCut("classID==1"));
  ...
  for(int j=0; j<=points;j++)
    {
      cut=-0.4 + 0.8*j/points;
      TCut TheCut =TCut(bdtvar + Form("> %f ",cut));


      TH1F *Cutted_DVCS = new TH1F("Cutted_DVCS", "Data1", 100,0,0);
      signal_Data->Project("Cutted_DVCS", bdtvar,TheCut  + TCut("classID==0") );
 
      TH1F *Cutted_Pi = new TH1F("Cutted_Pi","Data2",100,0,0);
      signal_Data->Project("Cutted_Pi", bdtvar,TheCut + TCut("classID==1"));
...
void Recreate_ROC(TTree* signal_Data,TTree* signal_DVCS, TTree* signal_pi){

  TCanvas *c1 = new TCanvas("c1","Histograms");
  c1->SetGrid();


  //TGraph* gr1=Recreate_ROC_Data(signal_Data);
  TGraph* gr2=Recreate_ROC_Sim(signal_DVCS, signal_pi);
  //TGraph* gr3=Recreate_ROC_Training();
  ...
  //c1->BuildLegend();
  c1->Print("Summary_Plots/Recreated_ROC.pdf");


}

void Excl_vars_aft_BDT(TTree* pDVCS_Data, TTree* pDVCS_Sim, TTree* pDVCS_bkg){
  gStyle->SetOptStat(0);
  ...
  TH1F *hist1c_Data2 = new TH1F("hist1c_Data2","",100,-0.02,0.02);
  TH1F *hist2c_Data2 = new TH1F("hist2c_Data2","",100,0,3);
  TH1F *hist3c_Data2 = new TH1F("hist3c_Data2","",100,-0.5,0.5);
  TH1F *hist4c_Data2 = new TH1F("hist4c_Data2","",100,-0.2,0.4);
  TH1F *hist5c_Data2 = new TH1F("hist5c_Data2","",100, -0.2,2);

  TCut cut = TCut("bestCandidateFlag==1 && _strip_Nuc_BDT > 0.0 && mm2_eg>0") + TCut("_strip_Nuc_BDT>get_bdt_event(strip_Xbj, strip_Q2, t_Ph)");
  ...
  pDVCS_bkg->Project("hist1c_Data2", p1, cut*TCut("Weight"));
  pDVCS_bkg->Project("hist2c_Data2", p2, cut*TCut("Weight"));
  pDVCS_bkg->Project("hist3c_Data2", p3, cut*TCut("Weight"));
  pDVCS_bkg->Project("hist4c_Data2", p4, cut*TCut("Weight"));
  pDVCS_bkg->Project("hist5c_Data2", p5, cut*TCut("Weight"));
  ...
  hist1c_Data2->SetLineColor(kBlue);
  ...
  hist2c_Data2->SetLineColor(kBlue);
  ...
  hist3c_Data2->SetLineColor(kBlue);
  ...
  hist4c_Data2->SetLineColor(kBlue);
  ...
  hist5c_Data2->SetLineColor(kBlue);
  ...
  c1->Print(TString("Summary_Plots/Excl_vars_aft_BDT.pdf"));
  
  
  hist1c_Data->Add(hist1c_Data, hist1c_Data2, 1, -1);
  hist2c_Data->Add(hist2c_Data, hist2c_Data2, 1, -1);
  hist3c_Data->Add(hist3c_Data, hist3c_Data2, 1, -1);
  hist4c_Data->Add(hist4c_Data, hist4c_Data2, 1, -1);
  hist5c_Data->Add(hist5c_Data, hist5c_Data2, 1, -1);
  ...
  c1->Print(TString("Summary_Plots/Excl_vars_aft_bkg.pdf"));
...
for(int i=1; i<=94; i++)
{
  ...
  {
  	Data_bkg->Add(Form("Analysis/bin_%i/TMostafa_pi0.root",i));
  }
...
/*
  ...
  std::cout<<"Recreate ROC curve"<<std::endl;
  Recreate_ROC(Data_Tree, DVCS_Tree, Pi0_Tree);
  std::cout<<"\n"<<std::endl;
  ...
  //BDT score
  std::cout<<"BDT score for Data, DVCS and Pi0"<<std::endl;
  Get_BDT_Score(DVCS_Tree, Pi0_Tree, Data_Tree);
  std::cout<<"\n"<<std::endl;
