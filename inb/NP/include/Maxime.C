...
bool BDT::Check_DVCS_cuts(TLorentzVector*& electron, TLorentzVector*& photon, TLorentzVector*& Nuc)
{
  ...
  double cos=(-electron->Px()*photon->Px() - electron->Py()*photon->Py() + (Ebeam - electron->Z())*photon->Pz())/(photon->P()*sqrt( pow(electron->P(),2) + Ebeam*Ebeam - 2*Ebeam*electron->P()*TMath::Cos(electron->Theta()) ) );
  ...
  delete beam;
  delete target;
  return t_Ph<0 && Q2>1 && W>2 && Xbj > 0 && Xbj < 1 && theta_gamma_e>5 && abs(t_Ph/Q2)<1;
...
void BDT::fill_DVCS_histograms(TLorentzVector*& electron, TLorentzVector*& photon, TLorentzVector*& Nuc, TH1F*& mm2_eg, TH1F*& Phi_Ph)
{
  TLorentzVector *beam = new TLorentzVector();
  TLorentzVector *target = new TLorentzVector();
  beam->SetXYZT(0,0,10.6,10.6);
  target->SetXYZT(0,0,0,0.938);
...
TH1* BDT::Maxime(TCut cut, double BDT_cut, int bin_number, int Nphi)
{
  ...
  ch1->Add(Folder + TString("Tagged_") + eppi0_name);
  ...
  if(recast)
    {
      TTree* Tree = chain ;
      TFile f(Form("Maxime_contamination/Maxime_pi0_recast_%i.root",bin_number),"RECREATE");
  ...
  if(add_BDT_Max)
    {
      Add_BDT_var_float(cut, Form("Maxime_contamination/Maxime_pi0_recast_%i.root",bin_number), TString("TMaxime_pi0.root"));
	Tag_P_Detected("Quality3_Data_eppi0_P.root", TString("TMaxime_pi0.root"));
	gSystem->Exec("rm " + Folder + TString("TMaxime_pi0.root"));
	gSystem->Exec("mv " + Folder + TString("Tagged_TMaxime_pi0.root ") + Folder + TString("TMaxime_pi0.root"));
    }
  ...
  if(means_maxi)
    {
      ...
      outFile <<"bin_number mean_phi error mean_t error mean_Q2 error mean_xB error"<<endl;
      outFile2 <<"bin_number entries_bef entries_aft before after"<<endl;

      for(int i=0; i<Nphi; i++)
	{
	  double Lbound[4] = {(360./Nphi)*i, -10., 0., 0.};
	  double Ubound[4] = {(360./Nphi)*(i+1), 0., 10., 1.0};
  ...
  TString outbsa;
  if(tag)
  	outbsa = Folder + TString("Maxime_Clean_Tagged.root");
  else
  	outbsa = Folder + TString("Maxime_Clean.root");
  	
  TFile *outputFile = new TFile(outbsa, "RECREATE");
  ...
  if(tag)
	c2->Print(Folder + TString("Maxime_BSA_Tagged.pdf"));
  else
