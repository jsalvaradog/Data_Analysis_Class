TH1* BDT::Get_Contamination_Mostafa(TCut cut, double BDT_cut, int Nphi, bool eta=false)
{
  ...
  cut2g = TCut(String_cut) + TCut("mm2_egg<1.5") + TCut("strip_eta_2DChi2 < 0.04 && abs(mm2_eNgg)<0.01"); // && (strip_Ph1_Theta <5 || strip_Ph2_Theta <5)");
  }
  else
  {
  ...
  //Create cut for 2gamma case
  cut2g = TCut(String_cut) + TCut("mm2_egg<1.5") + TCut("strip_Pi0_2DChi2 < 0.04 && abs(mm2_eNgg)<0.01"); // && (strip_Ph1_Theta <5 || strip_Ph2_Theta <5)");
  }
