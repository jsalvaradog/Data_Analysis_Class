...
void BDT::Training_on_bins(TString Data, int NBinsPhi=0, int bin=0, bool build=true, bool eta=false)//, TCut cut, TString DVCS, TString Pi0, vector<TString> vars)
{
  ...
  for(int k=k0;k<kN;k++)
      ...
      if(build)
	{
	  ...
	  Filter(TString("../../") + Data, cut + cut_bin, Data_name);
	  ...
	  if(pid_sys)
	  	Add_BDT_var(cut + cut_bin + cut_pid, Folder + TString("Tagged_") + Data_name, TData, Vars);
	  else
	  	Add_BDT_var(cut + cut_bin, Folder + TString("Tagged_") + Data_name, TData, Vars);
      ...
      Most=Get_Contamination_Mostafa(cut + cut_bin + cut_ref, BDT_value,Nphibins[k],eta);
      //boundaries.push_back(1);
      //boundaries.push_back(1);

      std::cout<<"\n Gettting Contamination Maxime way"<<endl;
      Maxi=Maxime(cut + cut_bin + cut_ref, BDT_value, bin_number,Nphibins[k]);
	...
	//To not display null measurements i.e. without events
      ...
      Compare_three(cut + cut_bin + cut_ref , "TMaxime_pi0.root", "TMostafa_pi0.root");
