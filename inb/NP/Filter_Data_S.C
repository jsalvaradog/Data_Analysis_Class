...
void Filter_Data_S(){
  ...
  //chain->AddFile(std::string("/work/clas12/jsalvg/RGA-Analysis/inb/P/Analysis/Tested_Quality_Data.root").c_str());
  for(int i=1; i<=64; i++)
  {
  ...
  TTree* Tree = chain ;
  TCut TheCut =TCut("bestCandidateFlag==1 && strip_Xbj <1 && strip_Xbj >0 && t_Ph <0 && strip_Q2 > 1.0 && strip_W > 2 && strip_Nuc_P > 0.35 && strip_El_P > 1.0 && strip_Ph_P>2  && strip_El_vz < 10 && strip_El_vz > -12 && TMath::Abs(Phi_Nuc - Phi_Ph) < 2 && TMath::Abs(t_Nuc - t_Ph) < 2 && TMath::Sqrt(Xbal * Xbal + Ybal*Ybal + Zbal*Zbal) <1.0  && _strip_Nuc_BDT > 0.0 && theta_gamma_X<1");
