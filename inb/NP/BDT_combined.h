...
public:
  ...
  Mbins[63]= TCut("bestCandidateFlag==1 && t_Ph<-0.8 && t_Ph>-100 && strip_Q2 > 5.0 && strip_Q2 < 15.0 && strip_Xbj > 0.55 && strip_Xbj < 1.0 ");
    ...
    //Folder="Combined/";
	dir="/work/clas12/jsalvg/RGA-Analysis/inb/NP/";
    ...
    DVCS="../Sample_Train_Sim_DVCS_NP.root";
    Pi0 ="../Sample_Pi_as_DVCS_NP.root";
    Data="../Quality3_Data_NP.root";
    TData="Tested_Quality_Data.root";
    TDVCS="Tested_DVCS.root";
    TPi0="Tested_Pi0.root";

    eppi0 = "../Quality3_Data_eppi0_NP.root";	
    maps_path = "../maps/";
...
theta_gamma_e > 5 && abs(t_Ph/strip_Q2)<1 && N_Ph<3 && mm2_eg > 0 && mm2_eg<2.0 && strip_Ph_Theta > 0 ) &&\
((strip_Ph_Theta > 5 && _strip_Nuc_BDT > 0.0) || (strip_Ph_Theta < 5 && _strip_Nuc_BDT > 0.04))";
  ...
  void Training_on_bins(TString Data, int NBinsPhi=0, int bin=0, bool build=true);
  ...
  TH1* Get_Contamination_Mostafa(TCut cut,double BDT_cut, int Nphi);
...
void BDT::Compute_Systematics(int bin=0){
    ...
    gSystem->Exec(TString("mkdir -p ") + Folder_0 + TString("Systematics_Phi/"));	
	Folder = Folder_0 + TString("Systematics_Phi/");
	gSystem->Exec(TString("rm -r ") + Folder + Form("bin_%i",bin));
	gSystem->Exec(TString("cp -r ") + Folder_0 + Form("bin_%i",bin) + TString(" ") + Folder + Form("bin_%i",bin)) ;
	Combine_FT_FD(bin,"../");
	gSystem->Exec(TString("find ") + Folder + Form("bin_%i/",bin) +TString(" -type f ! -name '*.txt' -exec rm -r -f {} \\;"));
